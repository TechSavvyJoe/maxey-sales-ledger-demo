...
test("two-year demonstration data ends today, supports historic views, and reloads cleanly", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("481 fictional demonstration sales are loaded")).toBeVisible();
  ...
  await expect(page.getByText(/481 previously removed records were restored/)).toBeVisible();
});
