...
import { currentMonthKey, monthKeyFromDate, shiftMonth, todayDateOnly } from "@/domain/date";
 ...
 * numeric contract.
 */
export function assertPersistableActualPaidByMonth(value: unknown): asserts value is Record<string, number | null> {
...
export function assertPersistableSaleNumbers(sale: Sale): void {
...
export function assertSaleHasPayPlanCoverage(sale: Sale, schedule: PayPlan[]): void {
...
export async function initializePublishedDemo(asOfDate = todayDateOnly()): Promise<boolean> {
  return db.transaction("rw", db.settings, db.sales, db.auditEvents, async () => {
    ...
    if (!profileApplied && isGeneratedDemoWorkspace
      ...
      const incoming = buildDemoSales(monthKeyFromDate(asOfDate), asOfDate, "three-year")
    ...
    await loadDemoSales(buildDemoSales(monthKeyFromDate(asOfDate), asOfDate, "three-year"), {
      historicDemoPlan: createPublicDemoHistoricPlan(asOfDate),
    });
    // A completed month tells the demonstration story immediately. This runs
    // only for a pristine first visit; migrations, resets, and later reloads
    // keep the month the visitor chose.
    const seededSettings = await db.settings.get(PROFILE_ID);
    if (seededSettings) {
      await db.settings.put({
        ...seededSettings,
        selectedMonth: seededSettings.selectedMonth === monthKeyFromDate(asOfDate)
          ? shiftMonth(monthKeyFromDate(asOfDate), -1)
          : seededSettings.selectedMonth,
        updatedAt: new Date().toISOString(),
      });
    }
    return true;
...
export async function persistSale(
  ...
  return db.transaction("rw", db.sales, db.settings, db.auditEvents, async () => {
    if (sale.status === "void") {
    ...
    const committed = normalizeSaleFinancing(persisted);
    await db.sales.put(committed);
    await db.auditEvents.add(
      createAuditEvent(
...
export async function persistSettings(settings: ProfileSettings): Promise<ProfileSettings> {
  ...
  return db.transaction("rw", db.settings, db.sales, db.auditEvents, async () => {
    ...
    if (!previous || previous.updatedAt !== settings.updatedAt) throw new SettingsWriteConflictError();
    const updated = {
      ...requested,
      createdAt: previous.createdAt,
      selectedMonth: previous.selectedMonth,
      selectedView: previous.selectedView,
      onboardingDismissed: previous.onboardingDismissed,
    ...
    (await db.sales.toArray()).forEach((sale) => assertSaleHasPayPlanCoverage(sale, schedule));
  ...
  await db.transaction("rw", db.sales, db.settings, db.auditEvents, async () => {
    ...
    if (needsHistoricCoverage && options?.historicDemoPlan) {
      ...
      if (nonDemoSales.length > 0) {
        throw new Error("Use a clean demo workspace before loading demonstration data.");
      }
    ...
    const existingById = new Map(
      (await db.sales.bulkGet(sales.map((sale) => sale.id)))
        .filter((sale): sale is Sale => Boolean(sale))
        .map((sale) => [sale.id, sale]),
    );
    const demoWrites: Sale[] = [];
    for (const sale of sales) {
      const existing = existingById.get(sale.id);
      if (!existing) {
        added += 1;
        demoWrites.push({ ...sale, profileId: PROFILE_ID });
        continue;
      }
    ...
    if (demoWrites.length > 0) await db.sales.bulkPut(demoWrites);
...
export async function replaceDatabaseFromBackup(
  ...
  // A restore is a new local commit. Keep its version newer than the row it
  // replaces even if the device clock moves backward or is held fixed.
  const currentSettings = await db.settings.get(PROFILE_ID);
  const timestamp = new Date(Math.max(
    Date.now(),
    currentSettings ? Date.parse(currentSettings.updatedAt) + 1 : 0,
  )).toISOString();
