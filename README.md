...
Maxey Sales Ledger is a production-grade, local-first sales and commission tracker for individual Bob Maxey Ford of Howell salespeople. It replaces the fragile month-tab workbook workflow with one clear month switcher, fast sales entry, deterministic calculations, weekly/monthly/year reporting, and user-controlled recovery backups.
...
- Fast guided Add Sale form with large required fields first, optional details kept one tap away, a live commission preview, three product outcomes, and a separate dealer-financing outcome.
...
- Product and dealer-financing outcome counts and rates based on valid delivered deals, alongside total F&I gross, F&I commission, and gross per delivery.
...
Deploy `dist/` to a static HTTPS host with the included cache/security headers or equivalent host configuration. `public/_headers` is applied automatically only by hosts that support that convention. Each salesperson gets an isolated personal workspace in that browser profile and should create regular JSON backups.
...
The app is intentionally not described as live cloud-synced, centrally backed up, manager-visible, multi-user, or payroll-authoritative. When a OneDrive- or Google Drive-synced folder is selected, Sales Ledger can confirm only the local verified file write; the selected sync client controls any later upload. The Google Drive web handoff also requires the salesperson to confirm the uploaded file appears in Drive. A shared team system requires authenticated accounts, role boundaries, a server-authoritative database, and a reviewed dealership retention policy.
